run_script_2 <- function(console_text, plot_label, step_script, checklist_labels, step_name) {
  # Define output paths
    ...
    if (file.exists(output_file)) {
      ...
      if (length(lines) > last_line_count) {
        ...
        for (line in new_lines) {
          ...
          if (grepl("Sampling", line)) {
            update_checklist_status(checklist_labels, "Sampled Data", "pass")
          } else if (grepl("Failed to read YAML file", line)){
            update_checklist_status(checklist_labels, "Sampled Data", "fail")
          }

          if (grepl("Recovering", line)) {
            update_checklist_status(checklist_labels, "Recovering Beta", "pass")
          } else if (grepl("should be character but is", line)) {
            update_checklist_status(checklist_labels, "Recovering Beta", "fail")
          } 
