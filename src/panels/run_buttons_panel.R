create_run_buttons_panel <- function(parent_frame) {
  ...
  run_step1 <- tkbutton(button_frame, text = "Run Step 1")
  run_step2 <- tkbutton(button_frame, text = "Run Step 2")
  run_step3 <- tkbutton(button_frame, text = "Run Step 3")
  
  tkpack(run_step1, run_step2, run_step3, side = "top", padx = 2, pady = 2, fill = "x")
  ...
  return(list(frame = button_frame, 
              buttons = list(step1 = run_step1, step2 = run_step2, step3 = run_step3),
