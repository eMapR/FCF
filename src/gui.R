...
build_help_text <- function() {
  paste(
    ...
    "semivariogram.png (Step 1): empirical semivariogram of residuals with the fitted",
    "nugget/sill/range curve, showing how spatial correlation decays with distance.",
    "chainImg.png (Step 2): one row per parameter (phi, sigma.sq, tau.sq); left column is",
    "the MCMC trace (check for mixing), right column is the posterior density.",
    "Step 3 plot: mean prediction map paired with each output layer in turn",
    "(mean, then SD) so you can compare the prediction against its uncertainty.",
    "",
    "Config editing:",
