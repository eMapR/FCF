...
build_help_text <- function() {
  c(
    ...
    "Displays messages produced by the application while a workflow step is",
    "running. Use it to follow the progress of an analysis and see whether a",
    "step completed successfully. If something goes wrong, check here first -",
    "errors, missing files, and configuration problems are reported here.",
    "",
    "## PLOT PREVIEW PANEL (bottom left)",
    "Displays diagnostic plots and results generated by the workflow. It",
    "updates automatically as steps finish, so what you see changes as you",
    "move through the workflow:",
    "",
    "-- After Step 1 - Variogram:",
    "    The empirical semivariogram generated from the residuals of the",
    "    initial regression model. Shows whether spatial structure remains",
    "    in the data and how it changes with distance.",
    "",
    "-- After Step 2 - Model Diagnostics:",
    "    Trace and density plots for three model parameters: phi (spatial",
    "    decay), sigma.sq (spatial variance), and tau.sq (nugget/error",
    "    variance). Trace plots show whether the model is sampling",
    "    consistently rather than drifting or becoming stuck. Density plots",
    "    show the estimated posterior distribution of each parameter.",
    "",
    "-- After Step 3 - Predictions:",
    "    The mean prediction map (estimated carbon across the landscape) and",
    "    the uncertainty map (standard deviation of that prediction). The",
    "    full prediction rasters are also saved to the output directory.",
    "",
    "## QUICK CONFIG SHORTCUTS PANEL (top right)",
    "Easy access to the settings you're most likely to change between",
    "analyses. For most routine runs, these may be all you need to modify.",
    "  Site: the site being analyzed. Must match a folder inside Data Dir.",
    "  Data Dir: the parent directory containing your sites' input data.",
    "  Output Dir: where model results, plots, and prediction rasters are saved.",
    "  Browse: opens a folder picker for Data Dir or Output Dir.",
    "  Apply To YAML: copies Site/Data Dir/Output Dir into the YAML Editor.",
    "  Pull From YAML: updates these fields from whatever is currently in",
    "  the YAML Editor (useful if you edited the YAML directly).",
    "",
    "## YAML EDITOR PANEL (middle right)",
    "The complete configuration used by the modeling workflow. Quick Config",
    "Shortcuts exposes the most commonly changed settings; this editor gives",
    "access to all of them. Start from the default or example configuration",
    "and only touch advanced parameters when necessary.",
    ...
    "  Save As: save the configuration as a new file - useful when",
    "  experimenting, since it preserves the original configuration.",
    "",
    "## RUN WORKFLOW PANEL (lower right)",
    "Where the analysis is performed. The workflow is four steps; each",
    "depends on the results of the one before it, so run them in order.",
    "Each row's status shows Pending, Running, Complete, or Failed.",
    "  Step 0: Check Inputs - checks configuration settings, input file",
    "  locations, site directory structure, boundary/plot/raster data,",
    "  coordinate reference systems, and raster coverage. Start every new",
    "  analysis here, and resolve any errors before continuing.",
    "  Step 1: Fit Variogram - fits an initial non-spatial regression and",
    "  examines spatial structure in the residuals.",
    "  Step 2: Fit Spatial Model - fits the Bayesian spatial model via MCMC",
    "  sampling. Can take substantially longer than Steps 0 and 1.",
    "  Step 3: Predict Outputs - applies the fitted model across the analysis",
    "  area, producing the final predictions and uncertainty.",
    "  Allow out-of-order runs: overrides the normal requirement that a step",
    "  finish before the next one runs. Leave unchecked unless you're",
    "  reopening an analysis where the required earlier outputs already exist.",
    "",
    "## HELP PANEL (bottom right)",
    "This panel. A quick reference for the FCF interface - descriptions of",
    "the major panels, workflow controls, configuration options, and",
    "diagnostic displays, without leaving the application."
  )
}
